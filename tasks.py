...
punctuation = np.array(['.', '?', '!', '"', "'", ';', ':', '-', '_'])

# Write a list comprehension to tokenize the text and remove punctuation
tokens = [''.join([char for char in word if char not in punctuation]) for word in text.split()]
...
def tokenize(string: str) -> list:
    punctuation = np.array(['.', '?', '!', '"', "'", ';', ':', '-', '_'])
    tokens = [''.join([char for char in word if char not in punctuation]).lower() for word in string.split()]
...
word_frequencies = {word: tokens_lowercase.count(word) for word in set(tokens_lowercase)}
...
def token_counts(string: str, k: int = 1) -> dict:
    ...
    tokens = [
        ''.join([char for char in word if char not in punctuation]).lower()
    ...
    # Return tokens that occur at least `k` times
...
def make_vocabulary_map(documents: list) -> tuple:
    # Check that each document in the list is a string
    if not all(isinstance(doc, str) for doc in documents):
        raise ValueError("All documents must be strings.")

    # Flatten all documents into a single list of tokens (lowercased and punctuation removed)
    all_tokens = []
    for doc in documents:
        all_tokens.extend([''.join([char for char in word if char not in np.array(['.', '?', '!', '"', "'", ';', ':'])]).lower() for word in doc.split()])
    
    # Get unique tokens and sort them
    unique_tokens = sorted(set(all_tokens))
    
    # Create token-to-integer and integer-to-token mappings
...
def tokenize_and_encode(documents: list) -> tuple:
    # Use the tokenize function and make_vocabulary_map
    tokenized_documents = [tokenize(doc) for doc in documents]  # Use the same tokenize function
    all_tokens = [token for doc in tokenized_documents for token in doc]  # Flatten the tokens
    
    # Create token-to-ID and ID-to-token mappings
    token_to_id = {token: idx for idx, token in enumerate(sorted(set(all_tokens)))}
    id_to_token = {idx: token for token, idx in token_to_id.items()}
    
    # Encode each document into a list of token IDs
    encoded_sentences = [[token_to_id[token] for token in doc] for doc in tokenized_documents]
...
# -----------------------------------------------
#def rnn_layer(w: np.array, list_of_sequences: list[np.array], sigma=sigmoid) -> np.array:
    # Split weights into W, U, and B
#    W = w[:9].reshape(3, 3)  # Assuming 3x3 for demonstration
#    U = w[9:18].reshape(3, 3)
#    B = w[18:21].reshape(1, 3)

#    outputs = []
#    for seq in list_of_sequences:
#        a = np.zeros(3)  # Initialize hidden state
#        for x in seq:
#            a = sigma(np.dot(W, x) + np.dot(U, a))  # Update hidden state
#        outputs.append(np.dot(B, a))  # Compute output for the sequence

#    return np.array(outputs).flatten()  # Ensure a NumPy array is returned
